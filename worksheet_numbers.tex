...
\newcounter{chrnum}
\newcounter{chrnumtwo}
%strlen is the number of single characters that prints in one line of the message
\newcount\numpages
\numpages=2
\newcount \strlen 
\strlen=99
...
%%ciphertable: has  three rows for letters and numbers
%    \fbox{
\begin{minipage}[b][1.25in][c]{8.3in}
{
\renewcommand{\arraystretch}{1.5}%sets stretch in height of row
\setlength\tabcolsep{5.5pt}%increases space between columns
...
\loop\redcellnum{\StrChar{\oneone}{\thechrnum}}\ifnum\thechrnum<\strlen\stepcounter{chrnum}\repeat\\%
\setcounter{chrnum}{1}\setcounter{chrnumtwo}{2}%%
...
\vskip 20pt
\hfil\lfs{Frequencies}\hfil
{\center{\begin{tabular}{| c | c | c | c |}
	\hline
    	\lfs{Number}&\lfs{Letter}\\
